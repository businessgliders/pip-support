// Shared helpers for resolving studio user profile photos / display names.
...
export const getPhotoForEmail = (email, allUsers = []) => {
  if (!email) return null;
  const u = allUsers.find((x) => x.email === email) || { email };
  return getPhotoForUser(u);
};