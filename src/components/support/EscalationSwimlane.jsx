...
export default function EscalationSwimlane({ currentUser, openSignal = 0, tickets = [] } = {}) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed md:bottom-24 md:top-auto top-1/2 -translate-y-1/2 md:translate-y-0 right-0 z-50 flex items-stretch transition-transform duration-300 ease-out ${
      ...
      {selected && (
        <div
          className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm flex items-start sm:items-center justify-center p-4 pt-20 sm:pt-4 overflow-y-auto animate-in fade-in duration-150"
          onClick={() => setSelected(null)}
        >
          <div
            className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[calc(100vh-6rem)] sm:max-h-[90vh] overflow-hidden flex flex-col my-auto"
