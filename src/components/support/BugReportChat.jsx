...
import { Bug, X, Send, Paperclip, Loader2, CheckCircle2, ImagePlus } from "lucide-react";
...
// Conversation steps
const STEPS = ["describe", "linkTicket", "ticketSelect", "platform", "urgency", "images", "review", "sending", "done"];
...
const PLATFORM_OPTIONS = ["Support Portal", "Website", "Mobile", "Email", "Other"];
...
export default function BugReportChat({ currentUser, tickets = [] }) {
  ...
  const handleClose = () => {
    setOpen(false);
    // Reset after close if we completed or never started
    if (step === "done") {
  ...
  const submitDescribe = () => {
    ...
    setStep("linkTicket");
    setTimeout(() => {
      pushAssistant("Got it. Is this related to an existing ticket?");
    }, 250);
  };
  ...
  const answerLinkTicket = (yes) => {
    ...
    if (yes) {
      ...
      setStep("platform");
      setTimeout(() => pushAssistant("Which platform did this happen on?"), 200);
    }
  };
  ...
  const pickTicket = (ticketId) => {
    if (ticketId === "__none__") {
      ...
      setTimeout(() => pushAssistant("No problem. Which platform did this happen on?"), 200);
      return;
    }
    ...
    setStep("platform");
    setTimeout(() => pushAssistant("Great. Which platform did this happen on?"), 200);
  };
  ...
  // Build a sorted list of active tickets for the picker
    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-5 right-5 z-50 group flex items-center gap-2 px-4 py-3 rounded-full bg-gradient-to-br from-[#b67651] to-[#a05a3a] text-white shadow-2xl hover:scale-105 active:scale-95 transition-all border border-white/30"
          title="Report a bug"
        >
      ...
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-5 right-5 z-50 w-[calc(100vw-2.5rem)] sm:w-[400px] h-[600px] max-h-[calc(100vh-2.5rem)] flex flex-col rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
            ...
            {step === "platform" && (
              <div className="flex flex-wrap gap-2 pt-1">
                {PLATFORM_OPTIONS.map(p => (
                  <Button key={p} size="sm" variant="outline" onClick={() => pickPlatform(p)} className="bg-white">
                    {p}
                  </Button>
                ))}
              </div>
          ...
          {/* Footer input (only for describe step) */}
          {step === "describe" && (
